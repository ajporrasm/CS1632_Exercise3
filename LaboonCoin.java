...
public class LaboonCoin {
	...
	//thought about string builder but this works
    public String createBlock(String data, int prevHash, int nonce, int hash) 
	{
	String chain = data + "|" + String.format("%08x", prevHash) + "|" + String.format("%08x", nonce) + "|" + String.format("%08x", hash);
	return chain;
    }
	...
	//hash as explained above; (n*ascii)+ascii
	...
	//casting chars as int's produces its ascii value
		for (int i = 0; i < charArray.length; i++)
		{
		n = ((n * (int)charArray[i]) + (int)charArray[i]); 
		}
	...
	String haxString = String.format("%08X", hash);
	
	for (int i = 0; i < difficulty; i++) 
	{
            if (haxString.charAt(i) != '0') 
			{
                validHash = false;
				return validHash;
            }
	...
	try 
	{
	    ...
	    if (difficulty < 0) 
		{
		System.out.println("Negative numbers not allowed, defaulting to difficulty = 3");
		difficulty = 3;
	    }
